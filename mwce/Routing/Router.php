...
class Router
{
        ...
        try {
            ...
            if (!$this->parseData['isCmd']) {
                ...
                if (!empty($_SESSION['mwclang'])) {
                    ...
                    if (!empty(Configs::buildCfg('dlang'))) {
            ...
            if (empty($this->parseData['controller'])) {
                self::$curController = Configs::buildCfg('defpage');
            }
            else {
                self::$curController = $this->parseData['controller'];
            }

            if (empty($this->parseData['action'])) {
                self::$curAction = 'actionIndex';
            }
            else {
                self::$curAction = 'action' . $this->parseData['action'];
            }
        }
        catch (\Exception $e) {
     ...
     * @param array $data
     */
    private function sessionParams($data): void
    {
        ...
        if (!empty($_SESSION['mwc' . $a . 'uid'])) {
            Configs::addParams('userID', (int)$_SESSION['mwc' . $a . 'uid']);
        }
        else {
     ...
     */
    public static function setCurController($controller): void
    {
     ...
     * @param string $action
     */
    public static function setCurAction($action): void
    {
     ...
     * @return string
     */
    public static function getCurController(): string
    {
     ...
     * @return string
     */
    public static function getCurAction(): string
    {
     ...
     */
    public function startModules(): void
    {
        try {
